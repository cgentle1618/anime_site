...
- Never commit or push automatically right after finishing a task. When a finished task is worth committing, show the proposed commit message and ask whether I want to commit. Only commit (and push) after I approve. Note that it's possible that we only commit once after multiple modifications.
