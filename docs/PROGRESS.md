...
| **The Clean action, validated against real data** | **Answered 2026-09-12, and running it found something else entirely.** The first real scan refused with a 503 - not a Clean bug: the backup sheet had been erased to headers, 16,774 rows across all 40 tabs, while the database was intact. Root cause was a defect in **Backup**, fixed at `fd9776c8`. Recovery: `pg_dump` to `~/anime_site_pre_sheet_restore_20260912.sql` (4.0 MB, verified), then Backup from the intact database, then the sheet verified row-for-row. Clean is now validated both directions on live data: **0 candidates and 0 anomalies** against 2081 entries (no false positives - the direct evidence that the `public_id` type mismatch is genuinely fixed, since that bug would have flagged all 2081), and **exactly 1 candidate**, correct, when one Media row was removed from the sheet data read-only in memory. The refusal that caught all this is the one decision I was least sure of: aborting the whole run rather than reporting per-tab. Pull's continue-and-report policy would have offered all 2081 entries for deletion | done |
...
| **The company machine can still erase the sheet** | `fd9776c8` fixes the guard only where it is checked out. That machine has the old code, and the sheet now holds the restored copy. **`git pull` there before running Backup**, not after | todo |
...
| **Unbreaking the dev app is one command, and it should run AFTER Phase B task 12** | `alembic upgrade head` against `anime_site_db`, **with the dev uvicorn stopped first and no other connection holding the schema**. Safe: `phaseb-session` exercised `n1a1accessmode` and `n1a2remarkauthor` against `anime_site_mig_check` - a `pg_dump`/`psql` restore of the real `anime_site_db` - forwards, backwards and forwards again, verifying the post-state by hand (four modes, `safe` carrying exactly the live guest role's field groups, every account holding four grants with one default, zero stale `field_group.*`/`label.*` rows, the remark index carrying `author_id`). Reason to wait rather than tidiness: **`n1a1accessmode`'s seed DERIVES the `safe` mode from whatever the guest role holds at the moment it runs.** That is deliberate - assuming the design's value would have published the other-sources list and other people's personal reviews to every logged-out visitor - but it means the migration reads live state, and it should read it once, deliberately, not as a side effect of somebody unbreaking a dev server | todo |
