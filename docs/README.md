...
| [deployment-selfhost.md](deployment-selfhost.md) | **production, as it runs**: an HP ProDesk 600 G4 mini at home serving `media.cg1618.com` through a Cloudflare Tunnel — the machine, the three containers, how it recovers unattended, networking, storage and cost. **Live; backups off the box are the only part not built** |
