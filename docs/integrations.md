...
| `aired.prop.from` + `aired.string` | `release_date` (`release_date_jp` for anime movies) | Precision comes from `aired.string`, the numbers from `prop`: `"Jul 6, 2026 to ..."` → `2026-07-06`, `"Jul 2026 to ?"` → `2026-07`, `"2026 to ?"` → `2026`. Both `aired.from` and `prop.from` are padded to 1 January when MAL knows only the year, so reading either alone would record a false date |
