...
| Backend | Media-sources cutover Task 12 (dropping `source_baha`/`baha_link`/`source_netflix`/`source_other`/`official_link`/`twitter_link`/`anilist_link` and their sheet columns) is written but held for explicit go-ahead — its downgrade is a deliberate `NotImplementedError`, and it is a breaking Sheets change (Backup must run from the newer-data machine before the other Pulls once it lands) | `app/models/{anime,anime_movie,movie,tv_show,cartoon,manga,novel,comic}.py`, `docs/data-actions.md` |
