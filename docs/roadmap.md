...
| Backend | `system_option_usage` has no Google Sheets tab, unlike its `system_option_scope` sibling — a `usage` row set on one machine does not reach the other via Backup/Pull | `app/services/pipelines/tabs.py` |
