...
tests/                          ← backend tests
  ...
  unit/
    test_utils.py               ← validate_episode_math, extract_mal_id, etc.
    ...
    test_checking_rules.py      ← has_missing_values, check_is_tv_completed
  api/
...
- `extract_mal_id`: valid MAL URL → ID; invalid → `None`
