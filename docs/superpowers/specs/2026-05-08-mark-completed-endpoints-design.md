...
1. **Duplicated business logic** — the backend already has `mark_tv_completed`, `mark_movie_completed`, and `mark_reading_completed` in `services/other_logics.py`. The frontend is re-implementing those rules inline.
...
|---|---|---|
| `routers/anime.py` | `POST /api/anime/{system_id}/complete` | Admin |
| `routers/anime_movie.py` | `POST /api/anime-movie/{system_id}/complete` | Admin |
| `routers/tv_show.py` | `POST /api/tv-shows/{system_id}/complete` | Admin |
| `routers/cartoon.py` | `POST /api/cartoon/{system_id}/complete` | Admin |
| `routers/movie.py` | `POST /api/movies/{system_id}/complete` | Admin |
| `routers/manga.py` | `POST /api/manga/{manga_id}/complete` | Admin |

### Endpoint logic (same pattern for all six)

1. Fetch entry by ID; return 404 if not found.
2. Call the appropriate `mark_*_completed(entry)` function from `services/other_logics.py`.
