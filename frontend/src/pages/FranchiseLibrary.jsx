...
import { getCoverUrl, FALLBACK_SVG, getSortName, getRatingWeight } from '../utils/anime'
...
function getFranchiseCover(franchise, animeDict, animesByFranchise) {
  ...
  const withCovers = franchiseAnime.filter(a => a.cover_image_file && a.cover_image_file !== 'N/A')
  if (withCovers.length === 0) return FALLBACK_SVG
  withCovers.sort((a, b) => (parseInt(b.release_year) || 0) - (parseInt(a.release_year) || 0))
...
function FranchiseCard({ franchise, coverUrl }) {
  const name = franchise.franchise_name_cn || franchise.franchise_name_en || franchise.franchise_name_romanji || 'Unknown Franchise'
  const ratingCls = RATING_COLORS[franchise.my_rating] || ''
  const expectCls = EXPECTATION_STYLES[franchise.franchise_expectation] || 'bg-gray-100 text-gray-500 border-gray-200'
  ...
  return (
    ...
    >
      <div className="relative bg-gray-100 overflow-hidden" style={{ aspectRatio: '2/3' }}>
        {franchise.my_rating && (
          <div className={`absolute top-0 left-0 ${ratingCls} text-[10px] font-black px-1.5 py-0.5 rounded-br-lg z-10 shadow-sm flex items-center gap-0.5`}>
            <i className="fas fa-star text-[8px]"></i>{franchise.my_rating}
          </div>
        )}
        <img
          src={coverUrl}
      ...
      <div className="p-2.5 flex flex-col gap-1.5 flex-1">
        <h3 className="font-bold text-gray-900 text-sm line-clamp-2 leading-tight" title={name}>{name}</h3>
        {franchise.franchise_expectation && (
          <span className={`self-start text-[10px] font-bold px-1.5 py-0.5 rounded border ${expectCls}`}>
            {franchise.franchise_expectation}
          </span>
        )}
      </div>
...
export default function FranchiseLibrary() {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        const [franchises, anime] = await Promise.all([fRes.json(), aRes.json()])
  ...
  const filteredAndSorted = useMemo(() => {
    ...
    let result = allFranchises.filter(f => {
      if (qClean) {
        const match = [f.franchise_name_cn, f.franchise_name_en, f.franchise_name_romanji, f.franchise_name_jp, f.franchise_name_alt]
    ...
    result.sort((a, b) => {
      if (currentSort === 'my_rating') {
        ...
        const diff = getExpectationWeight(a.franchise_expectation) - getExpectationWeight(b.franchise_expectation)
        if (diff !== 0) return diff
      }
      return getSortName(a, 'franchise').localeCompare(getSortName(b, 'franchise'))
    })
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3">
          <div className="flex flex-col sm:flex-row sm:items-center gap-3">
            <div className="flex-1 min-w-0">
              <h1 className="text-lg font-black text-gray-900 leading-none">Franchise Library</h1>
              <p className="text-xs text-gray-400 mt-0.5">
                {filteredAndSorted.length} franchise{filteredAndSorted.length !== 1 ? 's' : ''}
          ...
          {showFilters && (
            <div className="border-t border-gray-100 mt-3 pt-3 flex flex-wrap items-center gap-x-4 gap-y-2">
              <span className="text-xs font-bold text-gray-400 uppercase tracking-wider shrink-0">Type</span>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        {currentView === 'table' ? (
          <div className="flex flex-col items-center justify-center py-24 text-center">
            ...
            <h2 className="text-lg font-bold text-gray-700 mb-1">Table View Under Development</h2>
            <p className="text-sm text-gray-400">Switch to grid view to browse franchises.</p>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
            {filteredAndSorted.map(franchise => (
              <FranchiseCard
                ...
                franchise={franchise}
                coverUrl={getFranchiseCover(franchise, animeDict, animesByFranchise)}
              />
            ))}
          </div>
        )}
