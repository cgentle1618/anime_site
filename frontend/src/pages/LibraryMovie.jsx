  ...
  const filteredAndSorted = useMemo(() => {
    ...
    let result = allMovies.filter((m) => {
      ...
      if (
        ...
        return false;
      if (filters.movieType.size > 0 && !filters.movieType.has(m.movie_type))
        return false;
      if (filters.watchingStatus.size > 0) {
        const group = WATCHING_STATUS_GROUP[m.watching_status] || "Might Watch";
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
      ...
      <div className="flex flex-wrap items-center gap-3 mb-4">
        <div className="relative flex-1 min-w-48">
          ...
          <input
            type="text"
            placeholder="Search movies, franchise, director, year..."
            value={searchQuery}
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-4 mb-4 space-y-3">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-1.5">
              {["Finished Airing", "Not Yet Aired", "Airing"].map((v) => (
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-1.5">
              {["Reality", "Animation"].map((v) => (
                <FilterTag key={v} group="movieType" value={v} label={v} />
              ))}
            </div>
          </div>
          <div>
      ...
      {filteredAndSorted.length === 0 ? (
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3">
          {filteredAndSorted.map((m) => (
            <MediaCard key={m.system_id} type="movie" data={m} onUpdated={handleUpdated} />
          ))}
        </div>
      ) : (
          ...
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-gray-100">
              {filteredAndSorted.map((m) => {
                ...
                const subTitle = m.movie_name_en || "";
                const btnConfig = getStatusButtonConfig(m.watching_status);

                async function handleStatusToggle(e) {
                  e.stopPropagation();
                  const res = await fetch(`/api/movies/${m.system_id}`, {
                    method: "PATCH",
                ...
                return (
                  <tr
                    key={m.system_id}
                    onClick={() => navigate(`/movie/${m.system_id}`)}
                  ...
                  >
                    ...
                    <td className="px-4 py-2 text-xs text-center text-gray-500 border-r border-gray-100 hidden xl:table-cell">
                      {m.release_date_usa || "-"}
                    </td>
                    <td
