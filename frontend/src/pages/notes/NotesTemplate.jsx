...
// ─── Quote Section (API-backed) ─────────────────────────────
//
// The odd one out: quotes no longer live in the notes JSONB column, so this
// section talks to /api/quote directly instead of going through updateSection.
// Every other section on the page still writes one notes object.

function QuoteSection({ sectionKey, label, mediaType, entryId, isAdmin }) {
...
// ─── Meme Section (API-backed) ──────────────────────────────
//
// The second API-backed section, alongside quotes. A meme is an ordered list
// of text lines plus at most one image; a line carrying a quote_id is also a
// Quote, so it shows up in the Quotes section above as well.

export function MemeSection({ sectionKey, label, ownerType, ownerId, isAdmin }) {
  const queryClient = useQueryClient();
...
export default function NotesTemplate({
  ...
  const renderSection = (sec) => {
    ...
    switch (sec.type) {
      ...
      case "meme":
        // Like quotes, memes live in their own table rather than in notes.
        return (
          <MemeSection
            key={sec.key}
            sectionKey={sec.key}
            label={sec.label}
            ownerType={mediaType}
            ownerId={entity.system_id}
            isAdmin={isAdmin}
          />
        );
      case "quote":
        // Quotes live in their own table, so this section gets the entry
        // reference instead of a slice of the notes object.
        return (
          <QuoteSection
            key={sec.key}
            sectionKey={sec.key}
