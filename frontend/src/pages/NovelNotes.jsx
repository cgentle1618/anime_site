...
const SECTIONS = [
  ...
  { key: "craft", label: "巧思", type: "desc_links" },
...
function DescLinksForm({ value, onChange }) {
...
function DescLinksSection({ sectionKey, label, items, isAdmin, onUpdate }) {
  const [adding, setAdding] = useState(false);
    ...
    >
      {(items || []).map((item, i) => (
        ...
        >
          {editIdx === i ? (
            <div>
              <DescLinksForm value={editVal} onChange={setEditVal} />
              <SaveCancel onSave={saveEdit} onCancel={() => setEditIdx(null)} />
            </div>
          ) : (
      ...
      {adding && (
        <div className="border border-brand/20 rounded-lg p-2.5 bg-brand/5">
          <DescLinksForm value={draft} onChange={setDraft} />
          <SaveCancel
            onSave={commit}
            onCancel={() => {
              setDraft(emptyDescLinks());
...
export default function NovelNotes({ novel, isAdmin, onSave }) {
  const [notes, setNotes] = useState(novel.notes ?? {});
